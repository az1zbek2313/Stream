  ...
  return (
    <div style={{display:'flex',flexWrap:'wrap', flexFlow:'column'}}>
      <div style={{fontSize:'24px'}}>
        <b>Ishtirokchi: </b> 
        <span style={{color:'blue', fontSize:'28px', fontWeight:'600'}}>{copied}</span>{" "}
        <span style={{ color: "black" }}>||</span> <b>Webcam:</b>{" "}
        {webcamOn ? <span style={{color:'green'}}>ON</span> : <span style={{color:'red'}}>OFF</span>} <span style={{ color: "black" }}>||</span>{" "}
        <b>Mic:</b> {micOn ?<span style={{color:'green'}}>ON</span> : <span style={{color:'red'}}>OFF</span>}
      </div>
...
function MeetingView(props) {
  ...
  const joinMeeting = () => {
    let copied = localStorage.getItem('user')?JSON.parse(localStorage.getItem('user')):[];
    ...
    <div className="container">
      <h3 className="titleH3">
        ...
        <a
          ...
          style={{ color: "blue", cursor: "pointer", display:'flex', alignItems:'center', gap:'6px' }}
          className="href"
